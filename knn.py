...
# 3) PCA 2D scatter of TEST set to visualise separability
pca = PCA(n_components=2, random_state=RANDOM_STATE)
X_test_pca = pca.fit_transform(X_test)
plt.figure(figsize=(8,6))
palette = {0:"tab:green", 1:"tab:red"}
sns.scatterplot(x=X_test_pca[:,0], y=X_test_pca[:,1], hue=y_test.map({0:'Normal',1:'Attack'}),
                palette=['tab:green','tab:red'], alpha=0.6, s=30)
plt.title("PCA(2) projection of test set (colored by label)")
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.legend()
plt.tight_layout()
plt.savefig("pca_test_labels.png")
plt.show()
